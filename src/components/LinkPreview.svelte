  ...
  async function fetchMetadata() {
    ...
    try {
      ...
      try {
        ...
        console.log('[LinkPreview] Microlink failed, trying fallback');
      }
      ...
      if (data) {
        ...
        if (meta.title || meta.description || meta.image) {
          console.log('[LinkPreview] Metadata found:', meta);
          metaCache.set(url, meta);
        } else {
          console.log('[LinkPreview] No useful metadata found');
          error = true;
        }
      } else {
        error = true;
      ...
      console.error('[LinkPreview] Error fetching metadata:', e);
      error = true;
    } finally {
      loading = false;
    }
