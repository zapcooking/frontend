...
{#if open}
  ...
  >
    <dialog
      ...
      aria-modal="true"
      class="absolute m-0 top-1/2 left-1/2 px-2 md:px-8 pt-6 pb-8 rounded-3xl w-full md:w-[calc(100vw - 4em)] max-w-xl max-sm:h-[100%] sm:h-[calc(100vw - 4em)] overflow-y-auto -translate-x-1/2 -translate-y-1/2 bg-white dark:bg-gray-800"
      open
    >
      <div class="flex flex-col gap-6">
        <div class="flex justify-between">
          <slot class="self-center" id="title" name="title" />
          <button class="self-center cursor-pointer dark:text-gray-200" on:click={close}>
            <CloseIcon size={24} />
          </button>
        </div>
        <slot />
