<script lang="ts">
  ...
  async function applyPromoCode() {
    ...
    try {
      const response = await fetch('/api/membership/apply-promo', {
        ...
        body: JSON.stringify({ code, tier: 'cook', period: selectedPeriod })
      });
  ...
  async function proceedWithStripe() {
    ...
    try {
      ...
      const successUrl = `${baseUrl}/membership/cook-plus-checkout?payment=success&session_id={CHECKOUT_SESSION_ID}`;
      const cancelUrl = `${baseUrl}/membership/cook-plus-checkout?payment=canceled`;

      console.log('[Cook+ Checkout] Creating Stripe session...');
      ...
      if (!response.ok) {
        ...
        try {
          ...
          console.error('[Cook+ Checkout] Error response:', text);
        }
        throw new Error(errorMessage);
      }

      const data = await response.json();
      console.log('[Cook+ Checkout] Session created, redirecting to Stripe...');
  ...
  async function proceedWithLightning() {
    ...
    try {
      ...
      const data = await response.json();
      console.log('[Cook+ Checkout] Lightning invoice created');
  ...
  async function verifyLightningPayment(_preimage: string) {
    ...
    try {
      console.log('[Cook+ Checkout] Verifying Lightning payment...');
      ...
      console.error('[Cook+ Checkout] Verification error:', err);
...
<svelte:head>
  <title>Cook+ Membership Checkout - zap.cooking</title>
</svelte:head>
