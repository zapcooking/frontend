...
<div class="flex flex-col gap-4 max-w-full md:max-w-none">
  ...
  <div class="flex flex-col gap-3">
    ...
    {#if showTrend}
      ...
      >
        <span class="text-sm font-semibold leading-none" style="color: {TREND_GREEN}">
        ...
        >
          <defs>
            <linearGradient id="recipesTrendFill" x1="0" y1="0" x2="0" y2="1">
              <stop offset="0%" stop-color={TREND_GREEN} stop-opacity="0.22" />
          ...
          />
          {#if sparkLastPoint}
            <circle cx={sparkLastPoint.x.toFixed(1)} cy={sparkLastPoint.y.toFixed(1)} r="2" fill={TREND_GREEN} />
          {/if}
        </svg>
      </div>
    {/if}
