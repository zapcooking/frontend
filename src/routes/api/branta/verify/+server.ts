 ...
 *
 * GET /api/branta/verify?payment=<paymentString>[&encryptionKey=<secret>]
...
export const GET: RequestHandler = async ({ url, platform }) => {
  ...
  if (!qrText && !paymentString) {
    return json(
      { verified: false, error: 'payment or qr query parameter is required' },
      { status: 400 }
    );
  }
